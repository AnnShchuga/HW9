...
def export_with_commas():
    with open('HW9_Phonebook.txt', 'r', encoding='utf-8') as mf:
        lst = mf.readlines()
    s = ''
    for i, elem in enumerate(lst):
        if elem != '\n':
            ...
            with open('HW9_Phonebook_formatted.txt', 'w', encoding='utf-8') as mf:
                mf.write(s + '\n')
    s = ''
...
def answer(msg: telebot.types.Message):
    if msg.text == 'Экспорт (формат 1)':
        try:
            bot.send_document(chat_id=msg.from_user.id, document=open('HW9_Phonebook.txt', 'rb'))
            bot.register_next_step_handler(msg, answer)
            bot.send_message(chat_id=msg.from_user.id, text='Продолжим?',
                             reply_markup=buttons1)
        ...
        try:
            ...
            bot.send_document(chat_id=msg.from_user.id, document=open('HW9_Phonebook_formatted.txt', 'rb'))
            bot.register_next_step_handler(msg, answer)
            bot.send_message(chat_id=msg.from_user.id, text='Продолжим?',
                             reply_markup=buttons1)
        except Exception as e:
